...
- [ ] **Tesla Powerwall Certified Installer** — still blocked. Re-checked and re-confirmed blocked twice now specifically because requests came in to enable it (20 Sep 2026, and again 21 Sep 2026 when the request was marked "non-negotiable"). Application submitted 11 Sep 2026 (`mofuller@tesla.com` thread), but Tesla's own automated Partner Portal sent **"Tesla Onboarding Action Required: Your Onboarding Tasks have not been completed"** on **18 Sep 2026** — this is the most recent and most conclusive evidence found on either check, and it directly contradicts any claim that Tesla approval now exists. No certification confirmation has been received at any point checked. Badge component and asset remain built but not wired into any page. **This is not a negotiable point** — it depends on written confirmation from Tesla, which this inbox doesn't yet have; insisting doesn't change what Tesla's own system says. Do not enable until that confirmation exists. Note: Powerwall certification and Tesla Wall Connector installer status are separate — neither is confirmed, so the asset must not be used for any EV-charging claim either.
