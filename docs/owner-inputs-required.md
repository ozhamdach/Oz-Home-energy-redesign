...
- [ ] **15-Year Workmanship Warranty** — duration and workmanship-only scope are owner-confirmed; badge is built and enabled in preview (homepage, About page `#warranty`), with a minimal factual summary only (no exclusions, start dates, remedies or claim-expense terms invented). **Not publication-ready**: no full written, ACCC-reviewed warranty terms document exists yet for the badge to link to — check against https://www.accc.gov.au/business/selling-products-and-services/warranties (mandatory wording, contact/claim process, duration, remedies, expenses) before this goes live, and keep the website summary consistent with the proposal/contract/handover wording once that exists.
