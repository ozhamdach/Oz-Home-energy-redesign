...
- [x] **Ohme approved installer** — ✅ resolved: confirmed via the 11 Sep 2026 Ohme onboarding call (Julian Coxon) and the owner-supplied badge asset (`assets/original-photography/trust-badges/ohme-approved-installer.jpg`). Now enabled on the homepage trust strip and the EV charging page, scoped to Ohme EV charger installation only (not a general EV-brand claim).
- [ ] **Tesla Powerwall Certified Installer** — still blocked. Application submitted 11 Sep 2026 (`mofuller@tesla.com` thread); onboarding tasks (licence/insurance verification in the Tesla Partner Portal) were not complete as of the last correspondence found, and no certification confirmation has been received. Badge component is built (commented out, homepage) but must not be enabled until Tesla confirms in writing. Note: Powerwall certification and Tesla Wall Connector installer status are separate — neither is confirmed, so the asset must not be used for any EV-charging claim either.
- [ ] **15-Year Workmanship Warranty** — duration and workmanship-only scope are owner-confirmed; badge is built and enabled in preview (homepage, About page `#warranty`), with a minimal factual summary only (no exclusions, start dates, remedies or claim-expense terms invented). 16 of 17 owner decisions in the "OHE 15-Year Workmanship Warranty — Draft Terms" doc are now recommended-and-adopted pending your confirm/override; the correspondence address is the one still-missing fact (policy recommended — PO box/registered office, not a street address — but the literal text needs supplying). The About page's warranty summary has been updated in this branch to match the adopted eligibility-scope decision (removed "eligible projects," now "every installation — new system or retrofit, residential or commercial"); the badge caption ("15-Year" / "Workmanship Warranty") needed no change. **Still not publication-ready**: no full written warranty document is published yet for the badge to link to, and the summary deliberately doesn't yet add a "full terms" link/reference — there's nothing real to point to until the address is supplied and a final page/PDF exists. This is a drafting exercise against current ACL/ACCC guidance, not an ACCC-reviewed or ACCC-approved document — no such status exists, and nothing on the site should claim one.
