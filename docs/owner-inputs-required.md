...
- [ ] **15-Year Workmanship Warranty** — duration and workmanship-only scope are owner-confirmed; badge is built and enabled in preview (homepage, About page `#warranty`), with a minimal factual summary only (no exclusions, start dates, remedies or claim-expense terms invented). **Not publication-ready**: no full written warranty document exists yet for the badge to link to. A full draft (provider/contact details, coverage, start date, defect definition, remedies, claim process, expenses, exclusions flagged for owner decision, mandatory ACL guarantee wording) has been prepared for owner and legal review — see the "OHE 15-Year Workmanship Warranty — Draft Terms" doc. This is a drafting exercise against current ACL/ACCC guidance, not an ACCC-reviewed or ACCC-approved document — no such status exists, and nothing on the site should claim one. Keep the website summary consistent with the finalised terms and the proposal/contract/handover wording once approved.
